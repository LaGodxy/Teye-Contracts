...
impl ZkAccessHelper {
    /// Simple nullifier computation for testing
    pub fn compute_nullifier(
        ...
        // Simple hash of user and resource_id for basic nullifier
        let user_bytes = user.to_contract_storage_bytes();
        buf.extend_from_slice(&user_bytes);
        buf.extend_from_slice(resource_id);
        
        // Create simple hash
        let hash_bytes = env.crypto().keccak256(&user_bytes);
        hash_bytes.into()
    }
    ...
    pub fn create_request(
        ...
        for pi_item in pi.iter() {
            let mut buf = [0u8; 32];
            buf.copy_from_slice(*pi_item);
            pi_vec.push_back(BytesN::from_array(env, &buf));
        }
